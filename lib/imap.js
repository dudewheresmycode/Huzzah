...
router.post("/draft", function(req,res){
  ...
  MultipartRequest(req, function(err, mail_opts){
    ...
    mailObj.compile().build(function(err, message){
      imapStore.draft(req.account, message, req.query.uid, function(err,uid){
        res.json({success:!err, error:err, uid:uid});
      });
    });
  })
});
...
var imapStore = {
  ...
  draft: function(account, messageData, uid, callback){
    var config = optionBuilder(account);
    var imap = new Imap(config);
    imap.once('ready', function(err) {
      if(uid){
        console.log('remove previous draft', uid);
      }
      imap.append(messageData, {mailbox:"Drafts"}, function(err,uidplus){
        imap.end();
        callback(err, uidplus);
      });
    });
    imap.once('error', function(err) {
      console.log("Error", err);
